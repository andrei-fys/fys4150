...
void update_local_energy(double , double * , double * , double , double , double &, double &);
void update_potential_energy(double , double * , double * , double , double , double &);
void update_kinetic_energy(double , double * , double * , double , double , double & );
...
int main(int argc, char* argv[]){
	...
	double mean_potential_energy = 0.0;
	double kinetic_energy = 0.0;
	double mean_kinetic_energy = 0.0;
	...
	for (int j=0; j < 10; j++) {
	for ( int i=0; i < n; i++ ){
		Metropolis(MC_samples_step, omega, alpha, R1, R2, R1_new, R2_new, R12_new,  W,  local_energy, h,  expectation_energy, 								expectation_energy_squared, MC_rejected_prosent, MC_accepted_prosent, variance,  R12, mean_distance, beta, 								potential_energy, mean_potential_energy, kinetic_energy, mean_kinetic_energy);
	...
	potential_energy =0;
	mean_potential_energy =0;
	mean_distance = 0;
	kinetic_energy= 0;
	mean_kinetic_energy =0;
	R1[0] = 1.0;
	R1[1] = 1.0;
	R1[2] = 1.0;
	R2[0] = -1.1;
	...
	R2[2] = 1.1;
	//##############################
	Metropolis(MC_samples, omega, alpha, R1, R2, R1_new, R2_new, R12_new,  W,  local_energy, h,  expectation_energy, 							expectation_energy_squared, MC_rejected_prosent, MC_accepted_prosent, variance,  R12, mean_distance, beta, 							potential_energy, mean_potential_energy, kinetic_energy, mean_kinetic_energy);
	cout << "Final rejected  " << MC_rejected_prosent << endl;
	cout << "Expectation energy: " << expectation_energy << endl;
	cout << "Expectation potential energy: "<< mean_potential_energy/MC_samples << endl;
	cout << "Expectation kinetic energy: "<< mean_kinetic_energy/MC_samples << endl;
	cout << "Ratio for virial "<< (mean_kinetic_energy/MC_samples)/(mean_potential_energy/MC_samples)<< endl;
...
//	double R1new_sq = R1_new[0]*R1_new[0] + R1_new[1]*R1_new[1] + R1_new[2]*R1_new[2];
//	double R2new_sq = R2_new[0]*R2_new[0] + R2_new[1]*R2_new[1] + R2_new[2]*R2_new[2];
	R12_new = sqrt(pow(R1_new[0]-R2_new[0],2)+pow(R1_new[1]-R2_new[1],2)+pow(R1_new[2]-R2_new[2],2));
}
void update_potential_energy(double omega, double * R1, double * R2, double alpha, double beta, double & potential_energy){

	double R1_sq = R1[0]*R1[0] + R1[1]*R1[1] + R1[2]*R1[2];
	double R2_sq = R2[0]*R2[0] + R2[1]*R2[1] + R2[2]*R2[2];
	double R12 = sqrt(pow(R1[0]-R2[0],2)+pow(R1[1]-R2[1],2)+pow(R1[2]-R2[2],2));
	potential_energy = 0.5*omega*omega*(R1_sq + R2_sq) + 1/R12;
}

void update_kinetic_energy(double omega, double * R1, double * R2, double alpha, double beta, double & kinetic_energy){

	double R1_sq = R1[0]*R1[0] + R1[1]*R1[1] + R1[2]*R1[2];
	double R2_sq = R2[0]*R2[0] + R2[1]*R2[1] + R2[2]*R2[2];
	double R12 = sqrt(pow(R1[0]-R2[0],2)+pow(R1[1]-R2[1],2)+pow(R1[2]-R2[2],2));
	kinetic_energy = -0.5*omega*omega*(R1_sq + R2_sq)*(alpha*alpha) + 3.0*alpha*omega +
			+ 1/(2*(1+beta*R12)*(1+beta*R12))*(alpha*omega*R12 - 1/(2*(1+beta*R12)*(1+beta*R12)) - 2/R12 + 2*beta/(1+beta*R12));
}
void Metropolis(int MC_samples, double omega, double alpha, double * R1, double * R2,
				...
				double &variance, double & R12, double & mean_distance, double beta, double & potential_energy, 
				double & mean_potential_energy, double & kinetic_energy, double & mean_kinetic_energy){
	...
	for (int m=0; m < MC_samples; m++){ //MonteCarlo cycle
		...
		transition_probability(omega, alpha, beta, R1, R2, R1_new, R2_new, R12, R12_new, W);
			...
			if (W >= 1.0){
				...
				compute_distance(R1, R2, R12);
				update_potential_energy(omega,  R1, R2, alpha, beta, potential_energy);
				mean_potential_energy += potential_energy;
				update_kinetic_energy(omega, R1, R2, alpha, beta, kinetic_energy);
