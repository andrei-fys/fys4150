...
void write_prob_distr_file(int**, int);
...
[chaos] - ordered(0)/disordered(1) initial state" <<endl;
	...
	int absM2 = 0;
	int E_diff;
	int E_max = 2*N*N; //max energy of given lattice 
	double sum_energy, sum_magnetization, sum_energy2, sum_magnetization2,
		sum_absM, sum_absM2;
	//Matrix for probability distribution
	int **E_distribution = new int*[E_max];
	for (int i = 0; i < E_max; i++) {
		E_distribution[i] = new int[2];
	}
	for (int i = 0;i < E_max;i++){
		E_distribution[0][i] = 0;
		E_distribution[1][i] = 0;
	}
		...
		for (int m=0; m < MC_samples; m++){ //MonteCarlo cycle
			for (int k=0;k<N*N;k++){
				...
				if (E_diff <= 0){
					...
					probability_distribution(E_distribution, Energy_of_state, E_max);
					flip_counter++;
					MC_accepted++;
				} else {
...
//								sum_absM, sum_absM2);
		...
		//write_prob_distr_file(E_distribution, E_max);
		for (int i=0;i<E_max;i++){
			cout << E_distribution[0][i]<<","<< E_distribution[1][i]<<endl;
		}
		T += T_step;
	}
	...
	//for(int i = 0; i < E_max; ++i) {
	//	delete [] E_distribution[i];
	//}
	//delete [] E_distribution;
}
...
void write_prob_distr_file(int** Energies,int size){
	string filename = "Probability distribution";
	ofstream p_file;
	p_file.open(filename, std::ios::app);
	for (int i=0;i<size;i++){
	p_file << Energies[0][i] << "," << Energies[1][i] << endl;
	}
}
...
void probability_distribution(int** E_distribution, int E, int E_max){
	 ...
	 */
	int map = (E + E_max)/4;
	E_distribution[0][map] = E;
	E_distribution[1][map] += 1;
	//cout << E_distribution[0][map] << "," << E_distribution[1][map] << endl;
}
